...
const debug = require('debug')('pdf:db');
...
function createQueue (db, options = {}) {
  ...
  const createQueueMethod = function (func) {
    return function () {
...
function addToQueue (db, data) {
  const id = uuid();
  const createdAt = utils.getCurrentDateTimeAsString();

  const defaults = {
    meta: {}
  };

  if (!data.url || !utils.isValidUrl(data.url) || !utils.isAllowedUrl(data.url, queOptions.allowedUrls)) {
    return  db.pushToQueue(error.createErrorResponse(error.ERROR_INVALID_URL));
  }

  if (data.meta && typeof data.meta !== 'object') {
    return db.pushToQueue(error.createErrorResponse(error.ERROR_META_IS_NOT_OBJECT));
  }
...
function _markAsCompleted (db, id) {
  const completed_at = utils.getCurrentDateTimeAsString();
